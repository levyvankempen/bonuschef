...
- [x] 2.5 **Verified twice, and the second time is the more useful.** When an overlap existed: 192 offer rows in, **190 out** — `AH Aardappelschijfjes` (€0.27 clearance / €0.89 bonus) and `AH Mini krieltjes` each collapsed to one row, where summing both savings would have overstated that ingredient by €0.30. Re-checked later with no overlap present: 67 clearance + 103 bonus = **170 in, 170 out**, nothing to collapse. Both are correct outcomes, which is why the continuous guard is the `(store_id, product_link)` grain test rather than a row count — an overlap is a fact about a given day, not a property of the model
...
- [x] 9.5 The proposal step joins- [x] 9.8 **Decided by the operator, not implemented.** Asked whether unconfirmed matches should be marked on the page or excluded from ranking, given AH's search is confidently wrong about one time in five. The answer was to keep them: *"I know that unreviewed match is present, and it's okay that way. Brings more value to deliver recipes with possible error ingredient than bring recipes with no price."* So proposals continue to drive prices, and the mitigations are the ones that shipped instead — every ingredient names the product it rests on, and every line offers **Klopt niet** to correct it. Revisit only if a wrong price is ever read as an authoritative oneth a test that the page cannot name the work without offering it
- [x] 9.7 Verified live: **561 of 1,926 concepts matched in 90s**, mean ingredient coverage **3.3% → 46.5%**, ranked recipes **0 → 83**
- [x] 9.8 **Decided by the operator, not implemented.** Asked whether unconfirmed matches should be marked on the page or excluded from ranking, given AH's search is confidently wrong about one time in five. The answer was to keep them: *"I know that unreviewed match is present, and it's okay that way. Brings more value to deliver recipes with possible error ingredient than bring recipes with no price."* So proposals continue to drive prices, and the mitigations are the ones that shipped instead — every ingredient names the product it rests on, and every line offers **Klopt niet** to correct it. Revisit only if a wrong price is ever read as an authoritative one
